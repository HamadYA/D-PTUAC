...
from ltr.dataset import Lasot, Got10k, TrackingNet, SOT
...
#    coco_train = MSCOCOSeq(settings.env.coco_dir)
    ...
    # Train datasets
    SOT_train = SOT(settings.env.sot_dir)

    # Validation datasets
    SOT_val = SOT(settings.env.sot_dir)
    ...
    data_processing_val = processing.KLDiMPProcessing(search_area_factor=settings.search_area_factor,
                                                      ...
                                                      joint_transform=transform_joint)
    #,lasot_train,got10k_train
    # Train sampler and loader
    dataset_train = sampler.DiMPSampler([trackingnet_train,lasot_train,got10k_train], [1,1,1],
                                        samples_per_epoch=20000, max_gap=200, num_test_frames=3, num_train_frames=3,
    ...
    dataset_val = sampler.DiMPSampler([SOT_val], [1], samples_per_epoch=5000, max_gap=200,
                                      num_test_frames=3, num_train_frames=3,
                                      processing=data_processing_val)

    loader_val = LTRLoader('val', dataset_val, training=False, batch_size=settings.batch_size, num_workers=settings.num_workers,
                           shuffle=False, drop_last=True, epoch_interval=2, stack_dim=1)
    ...
    l1_loss = torch.nn.L1Loss()
    max=torch.nn.MarginRankingLoss()     
    #kd_loss=torch.nn.CosineEmbeddingLoss()    
    objective = {'bb_ce': klreg_losses.KLRegression(), 'test_clf': ltr_losses.LBHinge(threshold=settings.hinge_threshold),'l1_loss':l1_loss,'criterionGAN':criterionGAN,'max':max}

    loss_weight = {'bb_ce': 0.01, 'test_clf': 100, 'test_init_clf': 100, 'test_iter_clf': 400,'l1_loss':1,'criterionGAN':2,'kd':5}
    ...
    optimizer = optim.Adam([{'params': actor.net.classifier.filter_initializer.parameters(), 'lr': 5e-5},
                            ...
                            {'params': actor.net.feature_extractor.layer3.parameters(), 'lr': 5e-5},                            
                            {'params': actor.net.bb_regressor.parameters(),'lr': 5e-4}],
                           lr=2e-6)
                           
                          
    optimizer_D = optim.Adam([{'params': actor.net.feat_D1.parameters(), 'lr': 5e-4},
                            {'params': actor.net.feat_D2.parameters(), 'lr': 5e-4}],
                           lr=0.1*2e-4) 
    lr_scheduler = optim.lr_scheduler.StepLR(optimizer,  step_size=15, gamma=0.2)
    #pdb.set_trace()
    trainer = LTRTrainer(actor, [loader_train, loader_val], optimizer, optimizer_D, settings, lr_scheduler)
