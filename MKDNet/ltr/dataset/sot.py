...
class SOT(BaseVideoDataset):
    def __init__(self, root=None, image_loader=jpeg4py_loader, data_fraction=None):
        self.root = env_settings().sot_dir if root is None else root
        super().__init__('SOT', root, image_loader)
...
#                             low_memory=False).values
    ...
    def _get_frame(self, seq_path, frame_id):
        frame_path = os.path.join(seq_path, 'img', sorted([p for p in os.listdir(os.path.join(seq_path, 'img')) if os.path.splitext(p)[1] in ['.jpg','.png','.bmp']])[frame_id])
